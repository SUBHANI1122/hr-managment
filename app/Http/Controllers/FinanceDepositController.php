    ...
    public function store(Request $request)
    {
        ...
        if ($logged_user->can('add-deposit'))
        {
            ...
            try
            {
                ...
                if (!is_numeric($request->payment_method_id) || PaymentMethod::find($request->payment_method_id) == null) {
                    $payment = PaymentMethod::create([
                        'method_name'=>$request->payment_method_id,
                    ]);
                    $request->payment_method_id = $payment->id;
                }
                if (!is_numeric($request->payer_id) || FinancePayers::find($request->payer_id) == null) {
                    $finance = FinancePayers::create([
                        'payer_name' => $request->payer_id,
                    ]);
                    $request->payer_id =  $finance->id;
                }
                $data = [];
