...
class EmployeeController extends Controller {
    ...
    public function index(Request $request)
    {
        ...
        if ($logged_user->can('view-details-employee'))
        {
            ...
            $assignedCountries  = UserLocation::select('country_id')->where('role_id', $logged_user->role_users_id)->get();
                ...
                if ($request->company_id && $request->department_id && $request->designation_id && $request->office_shift_id){
                    $employees = Employee::with('user:id,profile_photo,username','company:id,company_name','department:id,department_name', 'designation:id,designation_name','officeShift:id,shift_name')
                        ...
                        if ($logged_user->role_users_id != 1){
                            $employees->whereIn('country', $assignedCountries);
                        }
                    $employees->orWhere('exit_date','0000-00-00')
                        ->get();
                    ...
                    if ($logged_user->role_users_id == 1){
                        $employees = Employee::with('user:id,profile_photo,username','company:id,company_name','department:id,department_name', 'designation:id,designation_name','officeShift:id,shift_name')
                            ->orderBy('company_id')
                            ->where('is_active',1)
                            ->where('exit_date',NULL)->orWhere('exit_date','0000-00-00')
                            ->get();
                    }else{
                        $employees = Employee::with('user:id,profile_photo,username','company:id,company_name','department:id,department_name', 'designation:id,designation_name','officeShift:id,shift_name')
                            ->orderBy('company_id')
                            ->where('is_active',1)
                            ->whereIn('country', $assignedCountries)
                            ->where('exit_date',NULL)->orWhere('exit_date','0000-00-00')
                            ->get();
                    }
                }
        ...
        if ($logged_user->can('store-details-employee'))
        {
            if (request()->ajax())
            {
                $validator = Validator::make($request->only('first_name', 'last_name', 'email', 'contact_no', 'date_of_birth', 'gender',
                    'username', 'role_users_id', 'password', 'password_confirmation', 'company_id', 'department_id', 'designation_id','office_shift_id','attendance_type','joining_date','country'),
                    [
                        'first_name' => 'required',
                        'last_name' => 'required',
                        ...
                        'company_id' => 'required|unique:companies,company_name',
                        'department_id' => 'required|unique:departments,department_name',
                        'designation_id' => 'required|unique:designations,designation_name',
                        'office_shift_id' => 'required|unique:office_shifts,shift_name',
                ...
                if (!is_numeric($request->company_id) || company::find($request->company_id) == null) {
                ...
                if (!is_numeric($request->department_id) || department::find($request->department_id) == null) {
                ...
                if (!is_numeric($request->designation_id) || designation::find($request->designation_id) == null) {
                ...
                if (!is_numeric($request->office_shift_id) || office_shift::find($request->office_shift_id) == null) {
                    $office_shift = office_shift::create([
                        'shift_name' =>$request->office_shift_id,
                        'company_id' => $request->company_id,
                    ]);
                    $request->office_shift_id = $office_shift->id;
                }
                ...
                $user['country']    = $request->country; //new
                $user['is_active'] = 1;
