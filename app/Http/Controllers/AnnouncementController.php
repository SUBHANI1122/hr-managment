...
//            foreach ($notifiable as $users){
//                Mail::bcc($users->email)->queue(new \App\Mail\AnnoucementEmail($data));
//            }

			return response()->json(['success' => __('Data Added successfully.'), 'details'=>$data]);
		}
    ...
    public function announcement_email(Request $request){
        $data['title'] = $request->title;
        $data['start_date'] = $request->start_date;
        $data['end_date'] = $request->end_date;
        $data['summary'] = $request->summary;
        ...
        return response()->json(['success' => __('Email Send .')]);
    }
