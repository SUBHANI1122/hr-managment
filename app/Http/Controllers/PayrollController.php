        ...
        if ($logged_user->can('view-paylist'))
        {
            if (request()->ajax())
            {
                ...
                return datatables()->of($employees)
                    ...
                    ->addColumn('over_time', function ($row)  use ($first_date, $last_date)
                    {
                        $total_hours = Attendance::where('employee_id', $row->id)->whereBetween('attendance_date', [$first_date, $last_date])->sum('overtime');
                        return $total_hours;
                    })
                    ->addColumn('over_time_amount', function ($row)  use ($first_date, $last_date)
                    {
                        ...
                        $total_hours = Attendance::where('employee_id', $row->id)->whereBetween('attendance_date', [$first_date, $last_date])->sum('overtime');
                        $to = Carbon::parse( $first_date);
                        $from = Carbon::parse($last_date);
                        ...
                        $diff_in_hours = $to->diffInHours($from);
                        $oneHoureBasicSalary = $basicsalary / $diff_in_hours ;

                        $overTimeSalary  = $oneHoureBasicSalary * $total_hours ;

//                        return $days."-".$diff_in_hours;
                        return round($overTimeSalary);
                    })
    ...
    public function payEmployee($id, Request $request)
    {
        ...
        if ($logged_user->can('make-payment'))
        {
            ...
            try
            {
                ...
                $data['net_salary']     = $request->payment_amount;
                $data['status']         = 1;
                $data['employee_id']    = $employee->id;
                ...
                if ((int)$account_balance < (int)$request->payment_amount)
                {
                    return response()->json(['error' => 'requested balance is less then available balance']);
                }

                $new_balance = (int)$account_balance - (int)$request->payment_amount;

                $finance_data = [];

                $finance_data['account_id'] = config('variable.account_id');
                $finance_data['amount'] = $request->payment_amount;
    ...
    public function PayamentEmail(Request $request){
        if ($request->type == "bulk"){
            $employees = Employee::whereIn('id', $request->all_checkbox_id)->get();
            foreach ($employees as $value){
                ...
                $employee = Employee::with(['user:id,username','company.Location.country',
                    ...
                    ->select('id','first_name','last_name','email','joining_date','contact_no','company_id','department_id','designation_id','payslip_type','pension_amount')
                    ->where('id',$value->id)->first()->toArray();


                // return $payslip->pension_amount;

                $payslip = Payslip::where('employee_id', $value->id)->first();
                ...
                Mail::send('email_templates.salary_slip', $employee, function($message)use($employee,$pdf) {
                    $message->to($employee["email"])
                        ->subject('Salary Slip')
                        ->attachData($pdf->output(), "salary_slip.pdf");
                });
            }
        }
        else{
            ...
            $employee = Employee::with(['user:id,username','company.Location.country',
                ...
                ->select('id','first_name','last_name','email','joining_date','contact_no','company_id','department_id','designation_id','payslip_type','pension_amount')
                ->where('id',$request->employee_id)->first()->toArray();


            // return $payslip->pension_amount;

            $payslip = Payslip::where('employee_id', $request->employee_id)->first();
            $total_minutes = 0 ;
            $total_hours = $payslip->hours_worked; //correction
            sscanf($total_hours, '%d:%d', $hour, $min);
            //converting in minute
